...
Usage:
    ...
    python3 mac_mlx --model mlx-community/Qwen2.5-7B-4bit --start-layer 20 --end-layer 28
...
# Code → (name, numpy dtype)  — bfloat16 has no numpy equivalent
DTYPE_NP = {
...
# ─── BFloat16 ↔ float32 conversion (numpy lacks bfloat16) ───────

def bf16_bytes_to_f32_np(data, shape):
    """bfloat16 raw bytes → numpy float32 array."""
...
def f32_np_to_bf16_bytes(f32_np):
    """numpy float32 → bfloat16 raw bytes."""
...
def raw_to_mx_array(data, dtype_code, shape):
    """Convert VTP raw payload to mx.array."""
...
def mx_array_to_raw(arr, target_dtype_code):
    """Convert mx.array → (raw_bytes, dtype_code)."""
...
def get_layers():
    """Return the transformer layer list from the model."""
    if _arch == "gpt2":
...
def load_model(model_name, start_layer, end_layer):
    """Load MLX model, free unused layers to save RAM."""
    ...
    print(f"Architecture: {_arch}, {_num_layers} layers, "
          f"keeping {start_layer}-{end_layer - 1}")
    ...
    # Free embedding/norm/lm_head (worker doesn't need them)
    if _arch == "llama":
...
def forward_layers(hidden, seq_len):
    """Forward hidden states through assigned layers."""
    layers = get_layers()
...
def handle_conn(conn, addr):
    """Handle persistent VTP connection (multiple forward requests)."""
    ...
    try:
        while True:
            ...
            # Header: start(H) end(H) seq_len(I) ndim(B) dtype(B) = 10 bytes
            ...
            if total_req <= 5 or total_req % 10 == 0:
                print(f"  step {total_req}: seq={seq_len} "
                      f"shape={list(shape)} fwd={fwd_ms:.1f}ms")
        ...
        if total_req > 0:
            ...
            print(f"[-] {addr}: {total_req} steps, "
                  f"avg fwd={avg:.1f}ms, total={elapsed:.1f}s")
        conn.close()
...
def main():
    parser = argparse.ArgumentParser(
        description="VTP MLX Compute Worker for VRAMancer")
