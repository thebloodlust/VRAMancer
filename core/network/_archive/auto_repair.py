...
class NodeAutoRepair:
    ...
    def start(self):
        ...
        self._thread.start()
        logging.info("[AutoRepair] Module d’auto-réparation démarré (daemon).")

    def stop(self):
        self.running = False
        if self._thread and self._thread.is_alive():
            self._thread.join(timeout=1)
        logging.info("[AutoRepair] Arrêt du module.")
            ...
            try:
                ...
                for node in nodes_snapshot:
                    if node.get("status") == "down":
                        logging.info(f"[AutoRepair] Nœud {node.get('host')} en panne, tentative de réparation...")
                        self._repair_node(node)
                        down_nodes.append(node)
                if down_nodes:
                    self._ha_redispatch(down_nodes)
            except Exception as e:  # pragma: no cover
                logging.info("[AutoRepair] Erreur monitor:", e)
    ...
    def _ha_redispatch(self, down_nodes):
        ...
        if not active_nodes:
            logging.info("[HA] Aucun nœud actif, cluster indisponible !")
            return
        rr_index = 0
        for node in down_nodes:
            ...
            for task in tasks:
                ...
                rr_index += 1
                logging.info(f"[HA] Tâche '{task}' transférée {node.get('host')} -> {target.get('host')}")
    ...
    def _repair_node(self, node):
        node["status"] = "active"
        logging.info(f"[AutoRepair] Nœud {node.get('host')} réparé.")
