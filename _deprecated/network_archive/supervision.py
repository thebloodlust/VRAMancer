...
class NodeSupervisor:
    ...
    def register_node(self, node):
        with self.lock:
            ...
            logging.info(f"[Supervision] Noeud {node.host} enregistré.")

    def update_status(self, node, status):
        with self.lock:
            self.status[node.host] = status
            logging.info(f"[Supervision] Statut de {node.host} : {status}")

    def monitor(self):
        logging.info("[Supervision] Démarrage de la supervision...")
        while True:
            with self.lock:
            ...
            time.sleep(5)
            logging.info(f"[Supervision] Statuts: {self.status}")
