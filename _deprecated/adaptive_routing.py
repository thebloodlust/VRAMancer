...
def route_layers(layers, node_caps):
    ...
    for i, layer in enumerate(layers):
        ...
        logging.info(f"[Routing] Couche {i} routée vers {best_node['host']} (VRAM dispo: {best_node['vram'] - best_node['used_vram']} MB)")
    return routed
...
def smart_preload(layers, node_caps):
    ...
    logging.info("[Preload] Plan de préchargement généré.")
    return preload_plan
...
def async_pipeline(tasks, node_caps):
    ...
    def worker(task, node):
        logging.info(f"[Async] Tâche {task['name']} lancée sur {node['host']}")
        time.sleep(task.get("duration", 1))
        logging.info(f"[Async] Tâche {task['name']} terminée sur {node['host']}")
    threads = []
    ...
    for t in threads:
        t.join()
    logging.info("[Async] Pipeline terminé.")
...
def dynamic_profiling(layers, node_caps):
    ...
    for i, layer in enumerate(layers):
        # Simule un profiling
        logging.info(f"[Profiling] Couche {i} | Taille: {layer.get('size', 512)} MB | Nœud: {node_caps[i % len(node_caps)]['host']}")
    logging.info("[Profiling] Routage ajusté.")
...
if __name__ == "__main__":
    ...
    w = b"dummy_weights"
    logging.info(compress_weights(w))
    tasks = [
