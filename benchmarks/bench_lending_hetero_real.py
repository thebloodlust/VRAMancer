...
def main() -> int:
    ...
    md += ["## What the table is showing", "",
           ...
           "(Rust P2P bypass for ≥ 512 KB activations, Strategy 2 pipelined "
           "double-buffer otherwise — see ``benchmarks/bench_kv_migration.py`` "
           "and ``benchmarks/bench_transfer_strategies.py``). ReBAR full-window "
           "(Strategy 1.7) was validated under a different Proxmox config in "
           "``docs/reports/REBAR_PROXMOX_BENCHMARK.md`` but is not active on "
           "this host (BAR0 = 8-16 MB, see T2.3 in BENCHMARK_RESULTS.md).",
           "",
