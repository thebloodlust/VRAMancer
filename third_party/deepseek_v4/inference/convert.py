        ...
        with safe_open(file_path, framework="pt", device="cpu") as f:
            for name in f.keys():
                ...
                for i in range(mp):
                    ...
                    # Force a heap copy to break mmap backing reference.
                    # safetensors returns mmap-backed tensors that keep file handles open.
                    # Without .clone(), os.remove() unlinks the file but the disk blocks stay
                    # allocated until all mmap references are dropped (possibly never before
                    # GC runs). .clone() + gc.collect() ensures immediate disk reclaim.
                    state_dicts[i][name] = new_param.clone()

    # Force GC to close all SafeTensors mmap handles (file handles from safe_open).
    # This ensures that the subsequent os.remove() calls actually free the disk blocks.
    gc.collect()

    os.makedirs(save_path, exist_ok=True)

    # Free disk space: delete input safetensors shards after loading into RAM.
    # This is safe because all tensors are already in state_dicts (CPU RAM via .clone()).
    # Without this, disk usage peaks at 2x model size (input + output).
    if os.path.realpath(save_path) != os.path.realpath(hf_ckpt_path):
        input_shards = glob(os.path.join(hf_ckpt_path, "*.safetensors"))
        if input_shards:
            print(f"Freeing {len(input_shards)} input shards from disk ({sum(os.path.getsize(f) for f in input_shards) / 1e9:.1f} GB)...")
            for shard in input_shards:
                os.remove(shard)
            print("Input shards deleted. Writing output files...")
